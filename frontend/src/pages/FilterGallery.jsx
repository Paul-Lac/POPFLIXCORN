  ...
  useEffect(() => {
    const fetchSearchResults = async () => {
      ...
      try {
        ...
        console.info("Search Results:", data.results);
          ...
          {searchResults && searchResults.length > 0 ? (
            searchResults.map((item) => (
              <CreateGallery
                ...
                navigate={() => {
                  let redirectUrl = "";
                  if (filmType === "tv" || filmType === "movie") {
                    redirectUrl = `/focus/${filmType}/${item.id}`;
                  } else if (filmType === "person") {
                    redirectUrl = `/acteur/${item.id}`;
                  }
